...
- [x] 全部 20 个 venue descriptor 已各选一个主题完成受控 Stage 1→4b 功能矩阵：19 个当前运行均下载并校验真实公开 PDF，以 `full_pdf` 各调用一次 Luna，再各调用一次且仅一次 one-shot Sol；NeurIPS 显式复用上条 hash-pinned 历史验收。严格汇总结果为 20/20，所有当前运行均通过 PDF→Luna→Sol lineage、CAS/manifest/hash、当前 deterministic content verifier 和 DB-bound legacy audit-order proof，且 reduce/audit/repair/retry 调用均为 0；本次发现的 audit runtime 顺序与持久化顺序不一致问题已修为统一按 `claim_id` 排序，旧 bundle 未改写、未二次调用模型。该矩阵的 Stage 1 为 approved one-record snapshot replay、Stage 2 为 `TEST_ONLY`，因此不冒充逐 provider 的 live transport 或 Stage 2 production release 证据；范围、主题、论文及哈希见 `docs/acceptance/venue-e2e-matrix-20260811.md` 与 `docs/acceptance/venue-e2e-matrix-20260811-evidence.json`。
