...
- Stage 1 字段补全器不得发现、加入或删除论文，只能对 primary census 已冻结的同一组 stable external IDs 回填字段；补全后的 ID 集与 primary 不完全相等时整单元 `unproven`。ICLR 以 DBLP conference-track TOC 冻结 membership，2016 用 arXiv Atom，2017–2019 用 OpenReview/ICLR 历史页与精确 arXiv 标题回退，2020–2025 用 ICLR 官方批量 virtual JSON；PDF 统一记录公开 `openreview.net/pdf?id=<forum_id>`。ICLR proceedings 不分配 article-level DOI，必须标记 `legitimately_absent/not_assigned_by_venue`，不得用 arXiv DOI 冒充会议 DOI。
- AAAI primary membership 继续由 OJS issue 枚举；摘要、DOI 和公开 PDF 优先以按年份分页的 Crossref registry 批量补全，并以 OJS OAI-PMH `GetRecord` 对缺项按 article ID 补齐。OAI/HTML 摘要缺失或明显截断时，允许只读取官方公开 PDF 第一页做确定性 Abstract→Introduction 区间抽取，不调用模型、不保存为 Stage 3 下载 artifact，并记录 `aaai_official_pdf:first_page_abstract` provenance。该异常回退不能扩大 membership。
- Stage 1 enrichment receipt 必须同时冻结 primary 与 hydration response/request hashes，并为 abstract、DOI、PDF URL 写字段级 provenance；required enrichment 任一记录仍缺 abstract/PDF，或 DOI 既缺失又未证明 `legitimately_absent` 时，严格模式只发布 incomplete receipt。
...
- [x] Stage 1 独立接口已对目录内全部 35 个 venue 完成 2016–2025 严格 membership census，所有适用 `venue × year` 均抵达 terminal cursor、稳定 ID 无重复且 parser 收支闭合；统一证据见 `docs/acceptance/stage1-decade-census-20260812.json`。ICLR、DAC、ICCAD 使用完整 DBLP proceedings TOC 作为声明的领域权威：ICLR 共 11,294 篇，DAC 排除 1 条 Frontmatter 后 2,668 篇，ICCAD 排除 1 条 Foreword 后 1,785 篇。ICLR 官网 accepted-event 与 proceedings 的 2022/2023 单条差异均为后来撤稿，不并入最终 proceedings。此项证明 membership 无遗漏，不冒充摘要、DOI 等可选字段全覆盖。
- [x] Stage 1 已加入 membership-preserving 字段补全契约，并完成 AAAI 2020（1,865/1,865 abstract、DOI、PDF）、ICLR 2020（687/687 abstract、PDF；DOI 依法标为主办方未分配）、ICLR 2024（2,260/2,260）与 ICLR 2025（3,704/3,704）的真实严格全量测试。ICLR 2017 在当前无人值守 OpenReview challenge 下仍有 28/198 篇无法由官网或精确 arXiv 标题匹配证明摘要，因此按设计保持 `incomplete`，不得宣称十年字段全覆盖；membership census 本身不受影响。
- [x] Stage 1 字段补全继续通过 IJCAI 2016（651/651 abstract、PDF；该卷早于 IJCAI DOI 系列，DOI 依法标为未分配）、IJCAI 2024（1,048/1,048 abstract、DOI、PDF）和 ICML/PMLR 2024（2,610/2,610 abstract、PDF；PMLR 未分配 article DOI）的真实严格全量测试。IJCAI 2017+ 使用年度 Crossref prefix 游标批量精确 ID join，PMLR 使用每 volume 单个官方 GitHub frontmatter 快照，不采用无法大规模并行的逐篇 detail 请求。证据见 `docs/acceptance/stage1-field-enrichment-20260813.md`。
- [x] NeurIPS 字段补全通过 2016（569/569 abstract、PDF；该时期未分配 article DOI）与 2024（4,493/4,493 abstract、DOI、PDF）真实严格全量测试。摘要来自官网年度 JSON 导出，2022+ DOI 通过 10.52202 年度 Crossref 游标、精确 proceedings container/title join 获取，个别注册题名变体回退官网 `citation_doi`；没有逐篇扫描整个年度。
- [x] JMLR 2024 字段补全通过 422/422 abstract、PDF 真实严格全量测试；官网 citation/BibTeX 未分配 article DOI，依法标为 `legitimately_absent/not_assigned_by_journal`。JMLR RSS 仅覆盖当前卷，历史年份不把 RSS 冒充全量快照，而按 4 QPS 受控并发读取官方详情页。
- [x] ACL 系列字段补全通过 ACL 2024（984/984 abstract、DOI、PDF）与 COLING/LREC-COLING 2024（1,567/1,567 abstract、PDF；年度 10.18653 注册表核验后 DOI 标为未注册）的真实严格全量测试。年度 Crossref prefix 游标按 Anthology stable ID 精确 join，XML 缺失摘要只对缺口论文从官方 PDF 首页面补取。
- [x] CVF 系列字段补全通过 CVPR 2024（2,716/2,716 abstract、PDF；2,715 DOI + 1 经注册表核验未注册）与 ICCV 2025（2,701/2,701 abstract、PDF；2,700 DOI + 1 未注册）的真实严格全量测试。年度 Open Access 索引直接给出 PDF；新年份以单个官网 virtual JSON 批量补摘要，DOI 以年度 DBLP proceedings 快照批量匹配，仅对残余缺口查询 Crossref，旧年份才按 4 QPS 回退官网详情页。
- [ ] EDA 与 Crossref 期刊字段补全已接入 membership-preserving DOI batch hydrator，但尚未达到十年全字段完成门槛。2024 实测 DAC 已由 OpenAlex 精确 DOI/标题回填至 370/370 abstract、370/370 DOI/PDF；ICCAD 为 234/239 abstract、239/239 DOI/PDF，剩余 5 条受 ACM Cloudflare、Semantic Scholar 429 与 OpenAlex 公共配额耗尽影响，保持 incomplete。JCIM 2024 已严格通过：805/805 DOI/PDF，740 条真实摘要，另 65 条经冻结题名规则证明为 masthead、publication information、correction、addition、editorial 等摘要不适用文献。Nature Machine Intelligence 2024 也已严格通过：184/184 DOI/PDF，166 条真实摘要，18 条由 Nature 官方文档类型证明为摘要不适用。Nature Biotechnology 2024 已严格通过：468/468 DOI/PDF，333 条真实摘要，135 条由 Nature 官方文档类型证明为 News in Brief、Research Highlight、Author/Publisher Correction、Correspondence、Podcast、News、Data Page 等摘要不适用。Angewandte Chemie 2024 已恢复为 5,170/5,170 DOI/PDF、4,817 条真实摘要，其余恰为 300 个封面/图文摘要、37 个 corrigendum、16 个 classifieds；批次超时以 warning 记录，严格 receipt 已通过。AISTATS/COLT/CoRL/EMNLP/NAACL/UAI 2024 代表年份已通过或完成恢复，其中 AISTATS 547/547 摘要与 PDF 已由官方详情 fallback 恢复。Science 2024 为 1,741/2,039 abstract、2,039/2,039 DOI/PDF；其 298 个新闻、信件、更正等页面受无人值守 Cloudflare challenge 影响，继续保持 incomplete。期刊路径先用 Europe PMC DOI 批量补全，Semantic Scholar 为可失败回退；Nature 系列再以独立、条款受控的 4 QPS 官方文章页补 `dc.description`，其它期刊可用精确标题 arXiv 批量残余回填。仍须完成 ICCAD 5 条、Science 缺口及其余期刊逐年验证后才能勾选本项。
