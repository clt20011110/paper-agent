...
from paper_agent.collector import CollectionOutcome
from paper_agent.errors import CollectionError, InputError, PublicationError
...
def _run_module(*arguments: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [sys.executable, "-m", "paper_agent", *arguments],
        cwd=REPO_ROOT,
        env=environment,
        capture_output=True,
        text=True,
...
def test_trusted_loader_loads_current_pmlr_adapter() -> None:
    ...
    assert adapter.__class__.__module__ == "paper_agent.adapters.pmlr"
...
def test_trusted_loader_errors_have_a_cause_without_scanning_or_leaking(
    ...
    assert calls == ["paper_agent.adapters.pmlr"]
